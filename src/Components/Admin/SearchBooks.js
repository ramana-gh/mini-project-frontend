...
function SearchBooks(props) {
  ...
  const handleInit = () => {
    ...
    setLoading(true);
    axios.get('https://department-library.herokuapp.com/admin/get-filters', {headers: {authorization: `Bearer ${token}`}})
    .then(response => {
      setLoading(false);
  ...
  const handleSearch = () => {
    ...
    axios.post('https://department-library.herokuapp.com/admin/search-books', {name: name.value, tags: tagArr, authors: authorArr, editions: editionArr, ratings: ratingArr}, {headers: {authorization: `Bearer ${token}`}})
    .then(response => {
      setLoading(false);
